...
export interface ILineageEmitterFactoryInput {
  ...
  /**
   * Override for the Knowledge Catalog Lineage API endpoint, sourced from the
   * --lineage-api-endpoint CLI flag or the DATAFORM_LINEAGE_API_ENDPOINT env
   * var. `undefined` means "use the default endpoint".
   */
...
export function createLineageEmitter(
  ...
  const options: IEmitterOptions = {
    ...
    apiEndpoint: input.apiEndpointOverride
  };
