...
suite("JiT support advanced", ({ afterEach }) => {
  ...
  test("JiT incremental pre/post ops support", async () => {
    ...
    const statementInc = incActionInc.tasks[0].statement;
  ...
  test("JiT incremental mode validation with consecutive runs", async () => {
    ...
    expect(secondAction.tasks[0].statement).to.include("SELECT 'inc_pre' as p");
    expect(secondAction.tasks[0].statement).to.include("SELECT 'inc_query' as q");
  });
