...
suite("JiT support dependencies", ({ afterEach }) => {
  ...
  test("JiT transitive dependency pruning", async () => {
    ...
    expect(actionB.tasks[0].compiledSql).to.include(`SELECT '\`${DEFAULT_DATABASE}.dataform.table_a\`' as ref_name`);
  });
  ...
  test("JiT to JiT dependency chain", async () => {
    ...
    expect(actionB.tasks[0].compiledSql).to.include(`SELECT '\`${DEFAULT_DATABASE}.dataform.jit_a\`' as ref_name`);
  });
});
