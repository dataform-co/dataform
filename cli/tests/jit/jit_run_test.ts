...
suite("run", () => {
  ...
  test("Global timeout cancels in-flight JiT worker", async () => {
    const { adapterInstance } = createMocks();
    ...
    const runner = new Runner(adapterInstance, executionGraph, {
      jitCompiler: (_req, _pdir, _adapter, _client, _timeoutMs, _opts, onCancel) =>
    ...
    expect(cancelCallbackInvoked).equals(true);
