...
On every pull request, CI runs [cloudbuild-canary.yaml](cloudbuild-canary.yaml) (`./scripts/run_canary_test`), which publishes a pre-release of `@dataform/core` under the `next` npm dist-tag and compiles the canary project in `tools/canary-project/` via the Google Cloud Dataform API (`dataform.googleapis.com`).
