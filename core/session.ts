  ...
  public compile(): dataform.ICompiledGraph {
    ...
    compiledGraph.tables.forEach(action => {
      ...
      if (!!action.type && !Object.values(TableTypes).includes(action.type)) {
        ...
        const message = `Error on dataset ${actionName}: Wrong type of table detected. Should only use predefined types: ${predefinedTypes}`;
        this.compileError(new Error(message));
      }

      // "where" property
      if (action.type === TableTypes.INCREMENTAL && (!action.where || action.where.length === 0)) {
        const message = `Error on dataset ${actionName}: "where" property is not defined. With the type “incremental” you must also specify the property “where”!`;
        this.compileError(new Error(message));
      }
      ...
      if (action.sqlDataWarehouse && action.sqlDataWarehouse.distribution) {
        ...
        if (
          ...
          const message = `Error on dataset ${actionName}: Invalid value for sqldatawarehouse distribution: "${distribution}"`;
          this.compileError(new Error(message));
        }
      }
      ...
      if (!!action.redshift) {
        if (
          ...
          const message = `Error on dataset ${actionName}: Missing properties in redshift config`;
          this.compileError(new Error(message));
        }
        ...
        const validatePropertyDefined = (
          ...
          if (!opts[prop] || !opts[prop].length) {
            const message = `Error on dataset ${actionName}: Property "${prop}" is not defined`;
            this.compileError(new Error(message));
          }
        };
        ...
        const validatePropertyValueInValues = (
          ...
          if (!!opts[prop] && !values.includes(opts[prop])) {
            const message = `Error on dataset ${actionName}: Wrong value of "${prop}" property. Should only use predefined values: ${utils.joinQuoted(
              values
            )}`;
            this.compileError(new Error(message));
          }
