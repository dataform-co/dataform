...
export function getCallerFile(rootDir: string) {
  ...
  if (!stack.length) {
    // This is likely caused by "compileErrors" being called from outside a session context.
    // If this is the case, the fix is to explicity pass the filename to "compileErrors".
    throw new Error(`Empty stack generated by "getCallerFile"; please report this issue.`);
  }
  while (stack.length) {
