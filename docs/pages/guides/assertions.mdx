...
To check for nulls on fields `a`, `b`, and `c` in a dataset `sometable` produced by another action, you can create a new file `definitions/sometable_not_null.assert.sql`:
...
For example, in a daily customer stats dataset, there should only ever be a single row for each combination of the `date` and `customer_id` fields.

Assuming a dataset called `daily_customer_stats` with fields `date` and `customer_id` which together should constitute a unique key, you can create a new file `definitions/daily_customer_stats_unique_keys.assert.sql`:
...
Assertions create named actions in your project that can be depended upon by using the `dependencies()` configuration statement.
