...
An assertion query is written to find rows that violate one or more rules. If the query returns any rows, then the test will fail.
...
If `a`, `b`, or `c` are null for any row in `sometable`, then this test will fail.

## Key uniqueness checks

Another common use case is to check that a particular field or combination of fields are unique in a table.
...
This query find any keys where there exists more than 1 row for that key, and thus are not unique.
...
This makes it easy to inspect the rows that caused the test to fail without increasing storage requirements or pulling any data out of your warehouse.
...
Given a default assertion schema `dataform_assertions` and an assertion file called `definitions/someassertion.assert.sql`:

```js
SELECT * FROM ${ref("example")} WHERE test > 1
```

Dataform will create a view called `dataform_assertions.someassertion` in your warehouse using the following query:

```js
CREATE OR REPLACE VIEW dataform_assertions.someassertion AS SELECT * from dataform.example WHERE test > 1
```

You can manually query this view to debug failing tests.
...
If you would like another table, assertion, or operation to only run if a specific test passes, you can add the test to that action's dependencies.
...
runs if `table1_checks` passes you could add it as a dependency in `definitions/table2.sql`:
