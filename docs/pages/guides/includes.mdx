...
If you are new to JavaScript, the examples below should cover most use cases.
<a target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/A_re-introduction_to_JavaScript">MDN</a> is a useful resource if you'd like to learn more.
...
<div className="pt-callout pt-icon-info-sign pt-intent-warning" markdown="1">
  ...
  using the <code>module.exports = {"{}"}</code> syntax as in the above example.
</div>
...
In the example below, there is function `countryGroup` that takes as input the name of the field and returns a CASE statement that maps country codes to country groups.
...
## An example include with parameters: groupBy

The example below defines a `groupBy` function that takes a number as input and generates a `GROUP BY` statement:
...
The `groupBy` function defined above is used in the following example:
...
Functions can be used to generate entire queries. This can be useful if you need to create several datasets which share a very similar structure.
