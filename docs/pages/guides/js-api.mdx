...
This example creates a dataset, an assertion, an operation, and a test in a single file, which if written in SQL would have required 4 seperate files.

## Setting properties on actions

Each of the global methods - `publish()`, `operate()`, `assert()`, and `test()` - return an object that can be used to configure that action. The API follows a builder syntax which can be seen in the following example:
