...
The following example SQL file defines a post operation to grant access to the table to specific groups, after the table is created.
...
This can be overriden by changing the file name to have a fully qualified table name. For example, to create a table called `example` in the schema `other_schema`, create the file `definitions/other_schema.example.sql`:
...
For writing JavaScript over multiple lines, create a comment block that starts with `/*js`:
