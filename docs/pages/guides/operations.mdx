...
If an operation creates a table or view that you would like to make available to other scripts, then you can use reference this operation like you would a normal table using the `ref()` function. Additionally operations may use the `self()` to create a table or view that matches the current file name.
