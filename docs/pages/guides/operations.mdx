...
Dataform that the output of this operations should be made available to callers of the `ref()` function.
...
The `self()` method can also be used to automatically reference the table or view denoted by the current file name. This makes it easy to ensure that they match:
...
## Running `VACUUM` commands (in Postgres or Redshift)

Redshift has a `VACUUM` command that can be used to improve the performance of some tables. This is a common use case for operations:
