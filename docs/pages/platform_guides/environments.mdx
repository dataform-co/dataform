...
Environments can be used to configure different variations of your schedules.
...
This configuration creates a `staging` environment which always runs the latest version of your project's code
(as exists on the `master` branch). When you have confirmed that the code works as expected, you can push that to production
by merging the `master` branch into the `production` branch.
