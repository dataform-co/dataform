...
Create a new file by clicking on the + button next to the `definitions` folder and create a new SQL file named `onboarding_dataset`.
...
You will see the right sidebar update with the name of the table/view, `onboarding_dataset`, a tag for the type - a view by default - as well as a validation message and two action buttons.
...
Upon successful completion of the run, a new view named `dataform.onboarding_dataset` will be created in your warehouse.
...
<div className="pt-callout pt-icon-info-sign pt-intent-none" markdown="1">
  ...
  <a href="../guides/tables">publishing datasets</a> and{" "}
...
You will see the right sidebar update with the name of the view, `onboarding_dataset2`. Clicking on `Show compiled` will display the fully qualified query to be run in your data warehouse.
...
<div className="pt-callout pt-icon-info-sign pt-intent-warning" markdown="1">
  ...
  However, since Dataform will run your project's scripts in dependency order, you can safely ignore
  these errors.
</div>
...
In bigger projects containing dozens or hundreds of tables with complicated interdependencies, the overview page can help you keep track of your project.
