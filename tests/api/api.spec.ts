...
suite("@dataform/api", () => {
  ...
  suite("run", () => {
    ...
    suite("dry-run property graph skip", () => {
      ...
      const successfulTableActionResult = {
        target: TABLE_TARGET,
        tasks: [{ status: dataform.TaskResult.ExecutionStatus.SUCCESSFUL, metadata: {} }],
        status: dataform.ActionResult.ExecutionStatus.SUCCESSFUL
      };
      const successfulGraphActionResult = {
        target: GRAPH_TARGET,
        tasks: [{ status: dataform.TaskResult.ExecutionStatus.SUCCESSFUL, metadata: {} }],
        status: dataform.ActionResult.ExecutionStatus.SUCCESSFUL
      };
      const withCompiledSql = (
        action: typeof successfulTableActionResult,
        compiledSql: string
      ) => ({
        ...action,
        tasks: [{ ...action.tasks[0], compiledSql }]
      });
      ...
      test("skips property graph when dry-run and a dep is an action in this run", async () => {
        ...
        expect(
          ...
          dataform.RunResult.create({
            status: dataform.RunResult.ExecutionStatus.SUCCESSFUL,
            actions: [
              withCompiledSql(successfulTableActionResult, TABLE_STATEMENT),
              {
                target: GRAPH_TARGET,
                tasks: [{ status: dataform.TaskResult.ExecutionStatus.SKIPPED }],
      ...
      test("runs property graph dry-run when no dep is an action in this run", async () => {
        ...
        expect(
          ...
          dataform.RunResult.create({
            ...
            actions: [
              withCompiledSql(successfulTableActionResult, TABLE_STATEMENT),
              withCompiledSql(successfulGraphActionResult, GRAPH_STATEMENT)
            ]
          }).toJSON()
        );
      ...
      test("runs property graph in non-dry-run mode even when deps are in this run", async () => {
        ...
        expect(
          ...
          dataform.RunResult.create({
            status: dataform.RunResult.ExecutionStatus.SUCCESSFUL,
            actions: [successfulTableActionResult, successfulGraphActionResult]
          }).toJSON()
        );
        verify(mockedDbAdapter.execute(GRAPH_STATEMENT, anything())).once();
      });
