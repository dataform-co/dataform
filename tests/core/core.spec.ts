...
class TestConfigs {
  public static redshift: core.ProjectConfig = {
    warehouse: "redshift",
    defaultSchema: "schema"
  };

  public static redshiftWithSuffix: core.ProjectConfig = {
    ...TestConfigs.redshift,
    schemaSuffix: "suffix"
  };

  public static redshiftWithPrefix: core.ProjectConfig = {
    ...TestConfigs.redshift,
    tablePrefix: "prefix"
  };

  public static bigquery: core.ProjectConfig = {
    warehouse: "bigquery",
    defaultSchema: "schema",
    defaultLocation: "US"
  };

  public static bigqueryWithDatabase: core.ProjectConfig = {
    ...TestConfigs.bigquery,
    defaultDatabase: "test-db"
  };

  public static bigqueryWithDatabaseAndSuffix: core.ProjectConfig = {
    ...TestConfigs.bigqueryWithDatabase,
    databaseSuffix: "suffix"
  };

  public static snowflake: core.ProjectConfig = {
    warehouse: "snowflake",
    defaultSchema: "schema"
  };
}
  ...
  suite("publish", () => {
    [TestConfigs.redshift, TestConfigs.redshiftWithPrefix].forEach(testConfig => {
      test(`config with prefix "${testConfig.tablePrefix}"`, () => {
        ...
        expect(t.enumType).equals(core.TargetType.TABLE);
        ...
        expect(t2.enumType).equals(core.TargetType.TABLE);
        ...
        expect(core.Target.create(t2.canonicalTarget).toJSON()).deep.equals({
          name: "example",
          schema: "schema2"
        });
        ...
        expect(t3.enumType).equals(core.TargetType.TABLE);
      });
    });
    ...
    test("canonical targets", () => {
      ...
      expect(
        [
          ...
          ...graph.declarations,
          ...graph.operations
        ].map(action => core.Target.create(action.canonicalTarget).toJSON())
      ).deep.equals([
        {
          database: "database",
          name: "dataset",
    ...
    test("validation_bigquery_pass", () => {
      ...
      expect(graph.tables[0].bigquery).to.deep.equals(
        profiles.BigQueryOptions.create({
          clusterBy: ["some_column", "some_other_column"],
          partitionBy: "some_partition",
          partitionExpirationDays: 7,
          requirePartitionFilter: false
    ...
    test("validation_type_inline", () => {
      ...
      expect(tableA.enumType).equals(core.TargetType.TABLE);
      expect(
      ...
      expect(tableB.enumType).equals(core.TargetType.INLINE);
      expect(
      ...
      expect(tableC.enumType).equals(core.TargetType.TABLE);
      expect(
