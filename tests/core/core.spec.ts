...
suite("@dataform/core", () => {
  ...
  [
    ...
    test(`name/prefix: "${target}"`, () => {
      ...
      const testTable = graph.tables.find(table => targetAsReadableString(table.target) === target);
...
suite("graph", () => {
  test("circular_dependencies", () => {
    ...
    expect(
      cGraph.graphErrors.compilationErrors.filter(item => item.message.match(/Circular dependency/))
        .length
    ).greaterThan(0);
  });
  ...
  test("missing_dependency", () => {
    ...
    const cGraph = session.compile();
    expect(
      cGraph.graphErrors.compilationErrors.filter(item => item.message.match(/Missing dependency/))
        .length
    ).greaterThan(0);
  });
