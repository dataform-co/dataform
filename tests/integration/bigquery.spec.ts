...
suite("@dataform/integration/bigquery", { parallel: true }, ({ before, after }) => {
      ...
      compiledGraph.tables = compiledGraph.tables.map(table => {
        if (
          ...
          dataform.Target.create({
            ...
            schema: "df_integration_test_run_caching",
            name: "example_view"
          })
        ) {
  ...
  suite("evaluate", async () => {
    test("evaluate from valid compiled graph as valid", async () => {
      ...
      const view = keyBy(compiledGraph.tables, t => utils.targetToName(t.target))[
      ...
      const table = keyBy(compiledGraph.tables, t => utils.targetToName(t.target))[
      ...
      const operation = keyBy(compiledGraph.operations, t => utils.targetToName(t.target))[
      ...
      const assertion = keyBy(compiledGraph.assertions, t => utils.targetToName(t.target))[
      ...
      const incremental = keyBy(compiledGraph.tables, t => utils.targetToName(t.target))[
