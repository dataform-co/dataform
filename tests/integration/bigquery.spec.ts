  ...
  suite("run", { parallel: true }, () => {
    test("project e2e", { timeout: 60000 }, async () => {
      ...
      await dbadapter.execute(
        `drop schema if exists \`${DEFAULT_DATABASE}.df_integration_test_project_e2e\` cascade`
      );
      ...
      const expectedFailedActions = [
        `${DEFAULT_DATABASE}.df_integration_test_assertions_project_e2e.example_assertion_fail`,
        `${DEFAULT_DATABASE}.df_integration_test_project_e2e.example_operation_partial_fail`
      ];
  ...
  suite("evaluate", async () => {
    test("evaluate from valid compiled graph as valid", async () => {
      ...
      const view = keyBy(compiledGraph.tables, t => targetAsReadableString(t.target))[
        `${DEFAULT_DATABASE}.df_integration_test_evaluate.example_view`
      ];
      ...
      const materializedView = keyBy(compiledGraph.tables, t => targetAsReadableString(t.target))[
        `${DEFAULT_DATABASE}.df_integration_test_evaluate.example_materialized_view`
      ];
      ...
      const table = keyBy(compiledGraph.tables, t => targetAsReadableString(t.target))[
        `${DEFAULT_DATABASE}.df_integration_test_evaluate.example_table`
      ];
      ...
      const operation = keyBy(compiledGraph.operations, t => targetAsReadableString(t.target))[
        `${DEFAULT_DATABASE}.df_integration_test_evaluate.example_operation`
      ];
      ...
      const assertion = keyBy(compiledGraph.assertions, t => targetAsReadableString(t.target))[
        `${DEFAULT_DATABASE}.df_integration_test_assertions_evaluate.example_assertion_pass`
      ];
      ...
      const incremental = keyBy(compiledGraph.tables, t => targetAsReadableString(t.target))[
        `${DEFAULT_DATABASE}.df_integration_test_evaluate.example_incremental`
      ];
...
`
    ...
    try {
      ...
      await dbadapter.execute(
        `drop schema if exists \`${DEFAULT_DATABASE}.df_integration_test_jit\` cascade`
      );
      ...
      const rows = await dbadapter.execute(`SELECT * FROM \`${DEFAULT_DATABASE}.df_integration_test_jit.jit_table\``).then(res => res.rows);
...
`
    ...
    try {
      ...
      // Drop dataset to start fresh
      await dbadapter.execute(
        `drop schema if exists \`${DEFAULT_DATABASE}.df_integration_test_jit_dry_run\` cascade`
      );
      ...
      const tables = await dbadapter.schemas(DEFAULT_DATABASE).then(schemas => {
        if (!schemas.includes("df_integration_test_jit_dry_run")) {
          return [];
        }
        return dbadapter.tables(DEFAULT_DATABASE, "df_integration_test_jit_dry_run");
      });
