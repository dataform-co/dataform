...
suite("@dataform/integration/postgres", { parallel: true }, ({ before, after }) => {
  const credentials = dfapi.credentials.read("postgres", "test_credentials/postgres.json");

  let dbadapter: dbadapters.IDbAdapter;

  before("create adapter", async () => {
    dbadapter = await dbadapters.create(credentials, "postgres");
  });
  ...
  test("run", { timeout: 60000 }, async () => {
    ...
    expect(
      ...
      // TODO: Update this error check to be correct.
